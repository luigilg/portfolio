...
<div class="p-10 min-h-screen bg-[#EAE8E1] relative">
    <div class="flex flex-col items-center">
        <h1 class="maintitle -tracking-[5px] gabarito font-black text-[10rem] text-[#2C2C2C] select-none cursor-default">ANIMAÇÕES</h1>
...
const items = projects
    ...
    .map(p => ({
        id: p.id,
        name: p.title,
        src: `../public${p.preview.src}`, 
        link: p.hasPage,
        speed: p.preview.speed,
        start: p.preview.start
    }));
...
onMounted(() => {
    ...
    const splitTextTitle = new SplitText('.maintitle', {
        type: 'chars',
        charsClass: 'char'
    });

    gsap.from(splitTextTitle.chars, {
        opacity: 0,
        delay: 0.3,
        x: 200,
        duration: 0.7,
