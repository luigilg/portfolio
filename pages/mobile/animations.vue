...
<div class="w-full min-h-screen bg-b-dark relative">
            ...
            <div :key="'fg-' + activeItemIndex" class="absolute inset-0 w-full h-full flex items-center justify-center p-[5vw]">
                 <template v-if="activeItemIndex !== null && items[activeItemIndex]">
                     <video 
                        ...
                        :src="resolvePath(items[activeItemIndex].src)"
                        autoplay loop muted playsinline 
                        class="w-full max-h-[50vh] object-contain shadow-2xl rounded-lg"
                     ></video>
                     <img 
                        v-else 
                        :src="resolvePath(items[activeItemIndex].src)" 
    ...
    <div class="fixed top-[13vh] w-full left-0 z-30 flex justify-center pointer-events-none">
        <transition name="slide-fade" mode="out-in">
             <div :key="activeItemIndex" class="flex flex-col items-center text-center w-[90vw]">
                 ...
                 <span class="funnel text-white/80 text-[3.5vw] mt-[2vh] uppercase tracking-widest font-bold animate-pulse">
...
useHead({
  ...
  meta: [
    { name: 'theme-color', content: '#282E32' }
  ]
});
...
onMounted(() => {
    ...
    const stHero = gsap.to('.hero-title', {
        top: '9vh', // Adjusted per user request (was 3vh)
        scale: 0.4, 
        y: 0, 
        scrollTrigger: {
            trigger: 'body',
    ...
    items.forEach((item, index) => {
        ...
        if (visualRefs.value[index]) {
             ...
             stInstances.push(tw.scrollTrigger);
        }
    });
